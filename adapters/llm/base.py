...
log = logging.getLogger("juhuo.llm_base")

# Rate limiter 暂时禁用（与同步 adapter 不兼容，待重构为全异步）
...
class LLMAdapter(ABC):
    ...
    def complete(self, request: CompletionRequest) -> LLMResponse:
        ...
        - 429 / 超时重试
        """
        # 限流保护（失败时 fallback 到直接调用）
        if HAS_RATE_LIMITER and self._rate_limiter is not None:
            import asyncio
            try:
                ...
                # asyncio event loop 冲突时 fallback 到直接调用
                log.warning(f"Asyncio conflict, falling back: {e}")
            except Exception as e:
                log.warning(f"Rate limiter error, falling back: {e}")
