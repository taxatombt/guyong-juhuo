...
# closed_loop 已移到懒加载（见下方 _LAZY_MAP 中的定义）
...
_LAZY_MAP = {
    ...
    # closed_loop 懒加载（避免触发 judgment._schema）
...
def __getattr__(name):
    ...
    if name in _LAZY_MAP:
        ...
        if attr_name is None:
            ...
            # 懒加载子模块本身（返回模块对象）
            from . import router as _router_mod  # unused placeholder

            import importlib
            return importlib.import_module(f"subsystems.judgment.{mod_name}")
