...
class FitnessEvolution:
    ...
    def record_outcome(
        ...
        # 写入verdict表（outcome_score 非空则存储）
        save_verdict(chain_id, task_text, correct, "fitness_evolution", outcome_score=outcome_score)
        ...
        try:
            from .self_evolver import sync_to_self_model, run_evolution_cycle
...
def record_judgment_outcome(
    ...
    """快捷函数：记录判断结果"""
    return get_fitness().record_outcome(chain_id, task_text, dimensions, weights, correct, outcome_score)
